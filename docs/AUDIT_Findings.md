...
| FR-BMS-05 | PARTIAL | **PASS** | Critical mask mở rộng 4→6/13 theo quyết định người dùng (commit `37ee2fb`); severity 1 được báo riêng, chỉ `sev>=2` mới là fault an toàn; recovery-bị-STALE-chặn (BUG-05) đánh giá là thiết kế cố ý, không phải bug |
...
- [x] **BUG-09 — Severity mapping không tách reporting và safety** — trước đây `sev>=1` bị đưa thẳng vào fault flags, làm warning có thể kích hoạt STOP. `bms_core.c` · **Medium** — _Đã chuẩn hóa: severity 1 đi vào `warning_flags` để hiển thị/log; severity 2/3 đi vào `alarm_flags` để giữ nguyên hành vi fault/STOP/ESTOP. Cùng một mã lỗi được dùng cho reporting, nhưng action của severity 1 luôn là INFO._
...
`test/host_protocol_sim/test_dwin_protocol_e2e.c` (rewritten 2026-08-30 to
the new API — 10 scenario cho DWIN: frame layout words/string/page, RX
dispatch + clear-back, byte-by-byte, resync, oversized-LEN, keyval-0,
scatter/diff cycle),
...
| FR-HMI-01/02 | PASS (protocol) — _Re-verified 2026-08-30: `Modules/hmi/dwin_protocol.*` rewritten against the real DGUS project (`ui/`), new `Modules/hmi/dwin_vp_map.h`. `DWIN_ParseRX()` resync/oversized-LEN behaviour preserved and re-tested._ |
| FR-HMI-03/04 | PASS — _Re-verified 2026-08-30: `App_Loop` section (4) rebuilt — 8-group scatter, diff-suppressed, real VP map; the screen button (`0x1042`) shares `app_action_button()` with PA15. TBD-01 closed._ |
...
- [ ] **I-17 — Setpoint ramp rates cần đo scope** — `charge_controller.c` `CHARGE_CTRL_VOLTAGE_RAMP_V_PER_S`=2 (post-close), `CHARGE_CTRL_VOLTAGE_PRECLOSE_RAMP_V_PER_S`=10 (pre-close), `CHARGE_CTRL_CURRENT_RAMP_A_PER_S`=5 · **Low** — _Added 2026-08-30 with FR-CTRL-19: `apply_charge_targets()` rate-limits the RISE of the commanded V/I (two-rate voltage: fast while arming the relay, slow after; decreases + EMERGENCY/FAULT unaffected). The two rates are educated starting values — the "reasonable" number depends on connector current rating, cable gauge, contactor make-current, and how fast the specific modules soft-start internally (TonHe/Maxwell/Lianming specify no ramp). Needs a clamp-meter/scope on the real DC bus at charge start to tune; move to `charge_cycle_config` once validated. Same class as I-07 / I-10._
- [x] **BUG-14 — NTC ADC đọc đứng yên từ lúc boot (jack over-temp protection vô hiệu)** — `BSP_ADC_Init()` gọi `HAL_ADC_Start_DMA()` đúng 1 lần, nhưng ADC cấu hình single-shot (`ContinuousConvMode=DISABLE`, `Core/Src/adc.c:54`) + DMA one-shot (`hdma_adc1.Init.Mode=DMA_NORMAL`, `adc.c:139`): 1 lần start → 1 bộ mẫu → cả ADC lẫn DMA tự disarm. Không có gì re-trigger. Hệ quả: `adc_buffer[]` đóng băng ở giá trị power-on; `BSP_ADC_GetTempC()` trả mãi mẫu cũ. Không chỉ sai hiển thị "JACK" trên DWIN — `apply_jack_temp_derating()` và fault jack over-temp (`charge_controller.c`) dùng chung đường này nên **soft-derating/fault theo nhiệt jack chưa bao giờ kích hoạt** (trừ khi mạch đã nóng sẵn lúc boot). Phần lọc mẫu lỗi (`adc_val==0 || >=4095 → NAN` + `isfinite()` guard ở cả 2 consumer) vốn đã đúng. · **Major — Safety/Functional** — _Fixed 2026-08-30 khi người dùng để ý JACK temp trên DWIN chỉ đổi 1 lần lúc boot. Thêm `BSP_ADC_Process(now_tick)` (`bsp_adc.c`): mỗi `BSP_ADC_RESAMPLE_INTERVAL_MS` (200ms — hằng số nhiệt NTC là giây) gọi `HAL_ADC_Stop_DMA` + `HAL_ADC_Start_DMA` để re-arm; gọi từ composition root (`App_Loop`, cạnh `CHG_LIB_Process`/`BMS_Process`). Chọn main-loop poll thay vì free-running ADC+ISR: dễ soi debugger, deterministic, coarse resample không mất gì với NTC (AGENTS.md 29). Cấu hình single-shot ADC + one-shot DMA mà re-arm dựa vào giờ được `check_ioc.py` pin (3 check mới: `ContinuousConvMode=DISABLE`, `hdma_adc1 Mode=DMA_NORMAL`, `DMAContinuousRequests=ENABLE`) để regen CubeMX không âm thầm đổi contract lấy mẫu. Verify: `-fsyntax-only` bsp_adc.c sạch, `check_ioc.py` 14/14, `check_architecture.py`, `test_logic`/`test_charge_e2e`/`test_dwin_protocol_e2e`/`test_pc_protocol_e2e` ALL PASSED, Release+rel-dwin build link (55.46% flash). Cần xác nhận trên hardware: cắm/rút NTC, hơ nóng jack → giá trị DWIN cập nhật liên tục + derating kích hoạt._
