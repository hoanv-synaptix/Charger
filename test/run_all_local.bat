...
echo [1/8] Running Static Analysis (check_ioc.py)...
python check_ioc.py
if %errorlevel% neq 0 (
...
echo [2/8] Running Native Unit Tests (test_logic.c)...
...
echo [3/8] Running Charger E2E Simulation (BMS + module CAN -> MCU -> app view)...
...
echo [5/8] Running DWIN Protocol E2E Simulation (byte stream -> dwin_protocol.c)...
...
echo [6/8] Running Alarm Subsystem E2E Simulation (BMS + module CAN -> alarm.c -> controller)...
...
echo [7/8] Building Firmware (Release preset)...
call build.bat
if %errorlevel% neq 0 (
...
echo [8/8] Firmware built successfully.
